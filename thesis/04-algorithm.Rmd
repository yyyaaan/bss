...
\begin{equation}
    ...
    \boldsymbol{R}_{\tau_1} = \boldsymbol{\Omega}_0 \boldsymbol{\Lambda}_{\tau_1} \boldsymbol{\Omega}_0' & \Leftrightarrow\ \boldsymbol{\Omega}_0^{-1} \boldsymbol{R}_{\tau_1} \boldsymbol{\Omega}_0^{-1'} = \boldsymbol{\Lambda}_1 ,  & \Lambda_1 \text{ is diagonal}\\
    \boldsymbol{R}_{\tau_2} = \boldsymbol{\Omega}_0 \boldsymbol{\Lambda}_{\tau_2} \boldsymbol{\Omega}_0' & \Leftrightarrow\ \boldsymbol{\Omega}_0^{-1} \boldsymbol{R}_{\tau_2} \boldsymbol{\Omega}_0^{-1'} = \boldsymbol{\Lambda}_2,  & \Lambda_2 \text{ is diagonal}\\
    \vdots \\
    \boldsymbol{R}_{\tau_l} = \boldsymbol{\Omega}_0 \boldsymbol{\Lambda}_{\tau_l} \boldsymbol{\Omega}_0' & \Leftrightarrow\ \boldsymbol{\Omega}_0^{-1} \boldsymbol{R}_L \boldsymbol{\Omega}_0^{-1'} = \boldsymbol{\Lambda}_{\tau_l},  & \Lambda_3 \text{ is diagonal}\\
    \end{cases}
    (\#eq:jade)
\end{equation}
...
\begin{equation}
\text{Approximate Joint Diagonalization Algorithm to find non-orthogonal } \boldsymbol\Omega_0 
\\ \begin{aligned}
...
\begin{equation}
\begin{aligned} 
\text{vec}( \widehat {\boldsymbol\beta_{2, \tau}})
&= \text{vec}(\boldsymbol{\mathcal E} \widehat{\boldsymbol\Omega_0} \widehat{ \boldsymbol\Lambda_\tau} \widehat{ \boldsymbol\Omega_0'})+ \text{vec}(\widehat{\boldsymbol\Omega_0} \widehat{ \boldsymbol\Lambda_\tau} \widehat{ \boldsymbol\Omega_0'} \boldsymbol{\mathcal E}')
...
The aforementioned three steps are able to find two major unknown matrix $\boldsymbol \Omega_0$ and $\boldsymbol{\mathcal E}$, and restoration is thus not challenging. The restoration is simply the inverse function of time-varying mix.
