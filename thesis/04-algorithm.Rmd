...
where, the underline means that the items will be considered as a whole in later steps.
...
The whitening indeed requires $\boldsymbol{R}_0$ to be positive semi-definite, though the autocovariance decomposition result \@ref(eq:covsep) and Equation \@ref(eq:olo) could not ensure such property. In practice, the Nearest Positive Definite Matrix (nearPD) algorithm by Bates and Martin Maechler [-@packageMatrix] if needed. Details about correcting non-positive semi-definite matrix were proposed by Knol and ten Berge [-@knol1989least] and have been further developed by Cheng and Higham [-@cheng1998modified]. 
