...
6. Loop for different algorithm to solve the TV-SOS problem, and report performance metrics.
...
| Sampling Frequency | $1:1,\ 2:1,\ \dots,\ 1024:1$ | $1:1,\ 2:1,\ \dots,\ 1024:1$ | $1:1,\ 2:1,\ \dots,\ 512:1$ | $1:1,\ 2:1,\  \dots,\ 512:1$ |
| Observed Length | $100000,\ 50000,\ \dots ,\ 98$ | $100000,\ 50000,\ \dots ,\ 98$ | $10000,\ 5000,\ \dots ,\ 40$ | $10000,\ 5000,\ \dots ,\ 40$ |

where matrix $\boldsymbol M = \begin{bmatrix} -3 & 6 & -6 \\ -4 &2.5 & 6\\ 9&2.1 &7 \end{bmatrix}$.

Aforementioned $tvSIR$ and $tvMD$, served as time-varying application of $SIR$ and $MD$ correpsondingly, are applied to measure the algorithm performance (in terms of accuracy and capabity of restoring the orginal signals), and results are reported in graph \@ref(fig:resultA) - \@ref(fig:resultD). Each sub-graph defines a specific simulation set with defined lag parameter used in LTV-SOBI algorithm. The curves illustrates the performance influenced by sampling frequencies, which are equivalent to observed signal length, and log-scale has been applied to the x-axis. Different algorithms can be distinguished by with color or line type.

\@ref(fig:resultA) and \@ref(fig:resultD) presumes to exihibit better performance measured by $tvSIR$ with larger value, i.e. the higher the points and curves the better. The missing values and other deviations will be deliberated in Chapter \@ref(discussion). With the two graphs, the new LTV-SOBI earns edge over Yereodr's original TV-SOBI in most cases, while the comparable adavantage seem to be dimished with increased observed length. On the other hand, the LTV-SOBI-alt algorithm is relatively insensitive to length, though it could not benefit from the larger size of observed information.
...
Using $tvMD$, \@ref(fig:resultA) and \@ref(fig:resultD) demostrates better performance with lower value, and LTV-SOBI-alt is comparably the best algorithm in most cases except for simulation Set III and larger observation usally leads to improved performance. The difference between LTV-SOBI and Yeredor's TVSOBI are esentially ignorable.
...
In conclusion, the extension of $MD$ and $SIR$ grants insights of algorithm performance; the simulation results suggest that LTV-SOS problem could yet be rather challenge, and LTV-SOBI algorithm is a comparably more applicable approach. Nontheless, the metric is not robust enough under different scenarios, and Chapter \@ref(dicussion) will attempt to detail such issue. Finally, the simulation results have to exclude the comparison with ordinary SOBI because of non-compatible metrics; SOBI would be penalized by the time-varying model, while benefiting from its ignorance of possible extreme values caused by $\boldsymbol{\mathcal E}$. Suppose the metric were acceptable with SOBI, the results prefer SOBI under large size of observations and LTV-SOBI in case observed length is relatively small. 