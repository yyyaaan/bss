...
```{r resultA, fig.cap='Performance measured by tvSIR for Set I and II (the larger the better)', out.width='98%', echo = FALSE}
library(tidyverse)
res_sum <- readRDS("/home/yan/bss/thesis/bss_res.rds")
m <- unique(res_sum$criteria)[4]
res_sum %>%
  filter(criteria == m, N > 90, method != "frjd", lag != 1, seriesT %in% c("Set I", "Set II")) %>%
  ggplot(aes(N, value, color=method, shape = method, linetype = method)) +
  geom_point(size = 1) + geom_line() + 
  scale_color_brewer(palette = "Set2") +
  scale_x_log10() +
  facet_grid(seriesT~lagT) +
  scale_y_continuous(name="tvSIR", limits=c(0, 9)) +
  theme_light()
```

```{r resultB, message=FALSE, results='hide', fig.cap='Performance measured by tvSIR for Set III and IV (the larger the better)', out.width='98%', echo = FALSE}
res_sum %>%
  filter(criteria == m, N > 30, method != "frjd", lag != 1, seriesT %in% c("Set III", "Set IV")) %>%
  ggplot(aes(N, value, color=method, shape = method, linetype = method)) +
  geom_point(size = 1) + geom_line() + 
  scale_color_brewer(palette = "Set2") +
  scale_x_log10() +
...
```{r resultC, fig.cap='Performance measured by tvMD for Set I and II (the smaller the better)', out.width='98%', echo = FALSE}
...
res_sum %>%
  ...
  scale_x_log10() +
  facet_grid(seriesT~lagT) +
  scale_y_continuous(name="tvMD", limits=c(0.7, 1)) +
  theme_light()
```

```{r resultD, fig.cap='Performance measured by tvMD for Set III and IV (the smaller the better)', out.width='98%', echo = FALSE}
res_sum %>%
  filter(criteria == m, N > 30, method != "frjd", lag != 1, seriesT %in% c("Set III", "Set IV")) %>%
  ggplot(aes(N, value, color=method, shape = method, linetype = method)) +
  geom_point(size = 1) + geom_line() + 
