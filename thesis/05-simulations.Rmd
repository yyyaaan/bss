...
In simulation configuration, the initial mixing matrix $\boldsymbol{\Omega}_0$ is first arbitrarily fixed to $\boldsymbol \Omega_{\text{sim}}$ as in \@ref(eq:sim_mat); then, 4 sets of simulation are parametered with differed time-varying factors and signal length, while the difference is only in terms of scale with details in Table \@ref(tab:four_sets). In each set, the ture source signal and mixture are thus fixed. 

Table: (\#tab:four_sets) Key Parametes of Simulation Set
|  | I | II | III | IV |
|---|---|---|---|---|
...
```{r sampling_rate, fig.show='hold', fig.cap='Illustration of 50:1 sampling rate: only signals at grey vertical bars are observed', out.width='80%', echo = FALSE}
...
Supposing the realistic scenario that true source is unobservable while mixture can be observed only at certain sampling rates (aka. sampling frequencies); for example, a piece of sound shall include 4800k samples per second as signal mixture, but the recording equipment can only sample at 48kHz, which means that merely 1 out of every 100 mixture is sampled as observation. Aiming to simulate such fact and evaluate how the observation length could affect LTV-SOBI performance, the simulation study further generates artificial observed mixture and corresponding source upon different sampling rates based on the same source and mixture. Figure \@ref(fig:sampling_rate) illustrates the sampling mechanism, and Table \@ref(tab:four_sets) summarized the sampling rates in each set. Consequently, multiple mixture of different lengths (as a result of sampling rates) are simulated within each set. In brief, instead of generating new pseudo sources and mixtures, the simulation study consider the sampling frequencies as a more robust alternative.

In the next step, LTV-SOBI algorithms along with various alternatives are applied to each generate observed mixture, and the results are compared against its corresponding true signal using tvMD and tvSIR, which finally leads to performance metric. 

Finally, 1000 similar simulation is performed to further elimated potential bias and outliers, and enable reporting of bootstrap results. Figure \@ref(fig:simulation_overview) overviews the simulation study in the manner of progress flow, and the full _R_ code is attached in Appendix \@ref(simcode).

```{r simulation_overview, fig.cap='Overview of Simulation Study Settings', out.width='89%', echo = FALSE}
