...
Although mathematically unnecessary, it is assumed that $\boldsymbol{\Omega}$ is a full-rank $p\times p$ matrix. In fact, an decrease in dimension of $\boldsymbol{\Omega}$ will allow fewer signal series (number of rows in $\boldsymbol z$) generated from the same observation $\boldsymbol{x}$. However, it is not necessary due to fact that the output carries real-world information, and the usefulness of series can be better determined using evidence of the signal itself. Consequently, it is a justified choice to force the mixing matrix to be full-rank.
