...
Previous sections have reviewed the TV-SOS model together with Yeredor's original TV-SOBI algorithm, while new approaches of LTV-SOBI and its alternatives are presented, together with a simulation study. Compared with Yeredor's original TVSOBI algorithm, LTV-SOBI eliminates one major problem of a non-positive semi-definite estimated covariance matrix as detailed in Section \@ref(algorithm). Yet, the LTV-SOBI algorithm does not always guarantee a solution and is particularly dependent on the observation. The most significant cause is the computational singularity, while in rather few cases, a negative-definite estimator also prohibits the algorithm of finding the nearest positive-definite matrix ('nearPD') and ultimately fails the whole algorithm. Despite the mixing matrix $\boldsymbol \Omega_0$ is assumed to be of full-rank, the non-singularity of every $\boldsymbol \Omega_t$ could not be secured as a result of small valued $\boldsymbol{\mathcal E}$. Sadly, the computational singular issue could occur in almost every step.
