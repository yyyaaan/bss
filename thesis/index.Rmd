...
| $\boldsymbol\Lambda_{\tau_i}$ | autocovariance given stationarity | $p\times p$ matrix that depends only on $\tau_i$ and assumed to be diagonal |
| $\boldsymbol R_{\tau_i} = \boldsymbol{\Omega \Lambda}_{\tau_i} \boldsymbol\Omega'$ | partially mixed autocovariance  | $p\times p$, a short-hand notation|
| $\boldsymbol K ^{(p,p)}$ | commutation matrix | $p^2\times p^2$ |
| $t=1,2,\dots,T$ | index of time | use in subscript. $t=0$ is a pseudo notation. | 
| $\tau_i$ | time lag | $i=1,2,\dots,l$ and $\tau_i\in L$. $\tau_0 \overset{\text{def}}= 0$ for convenience |
| $L = \{\tau_1,\tau_2,\dots,\tau_l\}$ | set of pre-selected lags | set of positive integers |
