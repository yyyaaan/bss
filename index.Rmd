...
## Alternative 1: Using $\boldsymbol \beta_1$ and $\boldsymbol \beta_2$
...
```{r test}
K3 <- matrixcalc::K.matrix(3) 
I3 <- diag(rep(1,9))
X <- K3 + I3

Beta_2 <- Beta_2x + t(Beta_2x)

lm(as.vector(Beta_2) ~ X - 1) -> temp
summary(temp)
as.vector(Beta_2x)
```
...
## Alternative 2: Using $\boldsymbol \beta_2$ and $\boldsymbol \beta_3$
...
A simple linear model solves $\boldsymbol\Omega$ and $\boldsymbol{\mathcal E}$ also follows.
