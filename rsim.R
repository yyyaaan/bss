...
sim_good_sources <- function(N, p = 4, sim_one = c("ecg", "sin"), sim_many = c("ar", "ma") ){
  ...
  for (rand in rand_types){
    if (rand == "ar" ) var <- arima.sim(list(ar=runif(1,-1,1)),N)
    if (rand == "ma" ) var <- arima.sim(list(ma=runif(1,-1,1)),N)
    if (rand == "sin") var <- sin((1:N)/(N/runif(1, 50, 200))) * runif(1, 0.5, 2)
    if (rand == "ecg") var <- generate_ecg((1:N)/(N/runif(1, 25, 75)), runif(1, 55, 105))
    z   <- cbind(z, var)
  }
  z <- as.ts(z)
  colnames(z) <- paste0("Series-", 1:p, "-", rand_types)
  
  z
}

# ECG function @ https://github.com/tanzirhasan/ECG_Simulation ------------
